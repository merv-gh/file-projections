...
OUT = "/tmp/df-exp"
PORT = int(os.environ.get("EXP_PORT", "8772"))
MAX_TURNS = 12
SAMPLE = os.path.join(ROOT, "fixtures", "dataflow-sample")  # prebuilt graph lives here
SRC = "src/main/java"
...
BUGS = [("sample/Tagger.java", 7, "mid = prefix", "mid = base"),
        ("sample/Pricer.java", 8, "total = fee", "total = net")]
TASK = ("A JUnit test is failing: App.summary(\"save\", 50) should be \"T-SAVE/$45\" but isn't. "
        "In each of Tagger.java and Pricer.java, one assignment reads the WRONG source variable, "
        "so the returned value is built from the wrong data. Trace the data flow into the "
        "returned value, find the two faulty assignments, fix them, and run the tests until they "
        "pass. Use the tools step by step; reply each step with a tool call.")

LIVE = {"model": MODEL, "status": "starting", "variants": [], "task": "fix 2 wrong-source assignments (Tagger, Pricer) so summary(\"save\",50)==\"T-SAVE/$45\" — Gradle/JUnit"}
def publish():
...
def open_data_flow(workdir, rel):
    """Trace the returned value's data flow with the REAL joern CPG var-flow lens (mode=joern,
    via the farm — no lexical fallback). Returns the source lines the value actually flows
    through (with real line numbers for edit_lines) and the declared locals that do NOT reach
    it — which exposes an assignment reading the wrong source."""
    full = os.path.join(workdir, SRC, rel)
    if not os.path.isfile(full):
        return f"no such file: {rel}"
    src = open(full).read().split("\n")
    rets = [i + 1 for i, l in enumerate(src) if re.search(r'\breturn\b', l)]
    if not rets:
        return f"no return statement in {rel}"
    retline = rets[-1]
    locals_ln = {}
    for i, l in enumerate(src):
        m = re.match(r'\s*[A-Za-z_][\w<>\[\].]*\s+([A-Za-z_]\w*)\s*=', l)
        if m:
            locals_ln.setdefault(m.group(1), i + 1)
    cands = re.findall(r'[A-Za-z_]\w*', src[retline - 1])
    var = next((c for c in cands if c in locals_ln), cands[-1] if cands else "x")
    stem = "/tmp/df-vf"
    if os.path.exists(stem + ".projection"):
        os.remove(stem + ".projection")
    r = subprocess.run([FPBIN, "-config", FARMCFG, "-analyzer", "joern-var-flow",
                        "-mode", "joern", "-source-root", SRC, "-file", rel, "-line",
                        str(retline), "-var", var, "-out", stem + ".projection"],
                       cwd=workdir, capture_output=True, text=True)
    if not os.path.isfile(stem + ".projection"):
        return f"joern var-flow failed (no fallback): {r.stderr.strip()[-200:]}"
    text = open(stem + ".projection").read(); os.remove(stem + ".projection")
    flow_lines, flow_text = set(), []
    for l in text.split("\n"):
        m = re.search(r':(\d+) :: (.*)$', l)
        if m:
            flow_lines.add(int(m.group(1))); flow_text.append(m.group(2))
    if not flow_lines:
        return f"joern var-flow: no CPG flow into `{var}` (no fallback)"
    ft = " ".join(flow_text)
    involved = sorted(n for n in flow_lines if 1 <= n <= len(src))
    out = [f"data-flow into `{var}` (returned at line {retline}) — real joern CPG flow:"]
    for n in involved:
        out.append(f"  line {n}: {src[n-1].strip()}")
    dead = sorted(v for v, ln in locals_ln.items() if v != var and not re.search(rf'\b{re.escape(v)}\b', ft))
    if dead:
        out.append(f"  declared locals that do NOT reach `{var}`: {', '.join(dead)}")
    out.append("  Fix the faulty assignment with edit_lines (write real Java).")
    return "\n".join(out)
...
def make_tools(variant, wd, state):
    ...
    impls = {"read_file": read_file, "search": search, "data_flow": data_flow,
    ...
    spec = [
        F("edit_lines", "Replace source lines [start,end] (1-based, inclusive) of a file with new_text. Use the line numbers shown by read_file/data_flow.",
    ...
    if variant == "proj":
        spec[:0] = [F("data_flow", "Trace a file's returned value with the real joern data-flow lens (path like sample/Tagger.java): returns the source lines the value flows through (with line numbers) and the declared locals that do NOT reach it. Fix the faulty assignment with edit_lines at its line.",
...
def system_prompt(variant):
    s = ("You are a coding agent fixing bugs in a small Java project (package sample: "
         "Tagger.java, Pricer.java, App.java; test AppTest.java). In two files an assignment "
         "reads the wrong source variable. Locate them, fix with edit_lines, and run_tests "
         "until they pass. Make minimal changes. Reply each step with a single tool call.")
    if variant == "graph":
        s += " Use the graph tools (code-review-graph) to explore symbols/structure, then read_file the region."
    if variant == "proj":
        s += (" Use data_flow(path) to see how a file's returned value flows and which locals do "
              "NOT reach it, then edit_lines the faulty assignment. No need to grep.")
    return s
...
<h1>data-flow lens (real joern) vs code-review-graph <span id=status class=live>…</span></h1>
...
<b>base</b>=grep+read · <b>graph</b>=code-review-graph traversal (real stdio MCP) · <b>proj</b>=joern var-flow lens via the farm (no fallback; edit its line numbers = two-way sync).
<b>inspect chars/calls</b> = code ingested to locate the faulty assignments.</p>
